...
CreateLBFunc_Def(InformedSelfishLB, "Greedy Selfish Task Reallocation");

InformedSelfishLB::InformedSelfishLB(CkMigrateMessage *m) : CBase_InformedSelfishLB(m) {
}

InformedSelfishLB::InformedSelfishLB(const CkLBOptions &opt) : CBase_InformedSelfishLB(opt) {
  lbname = "InformedSelfishLB";
  if (CkMyPe() == 0)
    CkPrintf("[%d] InformedSelfishLB created\n",CkMyPe());
  InitLB(opt);
}

void InformedSelfishLB::InitLB(const CkLBOptions &opt) {
  thisProxy = CProxy_InformedSelfishLB(thisgroup);
}

void InformedSelfishLB::Strategy(const DistBaseLB::LDStats* const stats) {
  if (CkMyPe() == 0) {
    CkPrintf("Starting InformedSelfishLB...\n");
  }
...
void InformedSelfishLB::CalculateMigrations() {
  ...
  if (!waiting_messages) {
    cur_iteration++;
    double tmp = my_load;
    CkCallback cb = CkCallback(CkReductionTarget(InformedSelfishLB, NextIteration), thisProxy);
    contribute(sizeof(double), &tmp, CkReduction::max_double, cb);
  }
}

void InformedSelfishLB::ReceiveTask(int host_id, double host_load, int task_id, double task_load,
    int arr_position) {
...
void InformedSelfishLB::ConfirmRecv(bool recv, int who, int arr_position) {
  waiting_messages--;
  if (recv) {
...
void InformedSelfishLB::NextIteration(double makespan) {
  ...
  if (cur_iteration >= kIterations) {
    ...
    if (!waiting_messages){
      contribute(CkCallback(CkReductionTarget(InformedSelfishLB, FinishLoadBalance), thisProxy));
    }
  } else {
    CalculateMigrations();
  }
...
void InformedSelfishLB::MoveTasks(int who, int msg_size, int* tids, int* targets) {
  int i = 0;
...
void InformedSelfishLB::ConfirmTasks() {
  waiting_messages--;
  if (!waiting_messages){
    contribute(CkCallback(CkReductionTarget(InformedSelfishLB, FinishLoadBalance), thisProxy));
  }
...
void InformedSelfishLB::FinishLoadBalance() {
...
bool InformedSelfishLB::DetermineMigrationSuccess(double host_load, double task_load) {
  bool ret_val = false;
...
void InformedSelfishLB::InsertMigrateInfo() {
  ...
  contribute(CkCallback(CkReductionTarget(InformedSelfishLB, FinalBarrier), thisProxy));
}

void InformedSelfishLB::FinalBarrier() {
  if (_lb_args.debug() > 2)
    CkPrintf("[%d] I have received a total of %d tasks\n", CkMyPe(), migrates_expected);
  ProcessMigrationDecision(msg);
}

void InformedSelfishLB::InformMigrations(int count) {
  migrates_expected += count;
}

#include "InformedSelfishLB.def.h"
